...
String.prototype.format = String.prototype.format || function (...args) {
    ...
    if (args.length) {
        args.forEach((element, i) => {
            result = result.replace(`{${i}}`, element);
        });
    }
    return result;
};

String.prototype.interpolate = String.prototype.interpolate || function (properties, from) {
    properties = properties && typeof properties === 'object' ? properties : {};
    const start = this.indexOf('{', from || 0);
    if (start === -1) {
        return this;
    } else {
        const end = this.indexOf('}', start);
        const subst = properties[this.substring(start + 1, end)] || `{${this.substring(start + 1, end)}}`;
        return `${this.substring(0, start)}${subst}${this.substring(end + 1)}`.interpolate(properties, end);
    }
};

String.prototype.equalsIgnoreCase = String.prototype.equalsIgnoreCase || function (str) {

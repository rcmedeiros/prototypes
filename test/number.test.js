...
describe('the random() function', function () {
    it('should return numbers within range', function () {
        const last = Number().pseudoRandom(100, 1000);
        for (let i = 0; i <= last; i++) {
            const min = Number().pseudoRandom(0, 100);
            const max = Number().pseudoRandom(1000, 10000);
            const num = Number().pseudoRandom(min, max);
    ...
    it('should swap range if max greater than num', function () {
        const last = Number().pseudoRandom(100, 1000);
        for (let i = 0; i <= last; i++) {
            const min = Number().pseudoRandom(1000, 10000);
            const max = Number().pseudoRandom(0, 100);
            const num = Number().pseudoRandom(min, max);
    ...
    it('If single positive number, should return numbers between 0 and the number', function () {
        const last = Number().pseudoRandom(100, 1000);
        for (let i = 0; i <= last; i++) {
            const single = Number().pseudoRandom(1, 100);
            const num = Number().pseudoRandom(single);
    ...
    it('Same numbers should return the number', function () {
        ...
        for (let i = 0; i <= last; i++) {
            const single = Number().pseudoRandom(1, 100);
            const num = Number().pseudoRandom(single, single);
            expect(num, `Expect ${num} to be ${single}`).to.be.equals(single);
        }
    });
    it('invalid values should return 0', function () {
        expect(Number().pseudoRandom(-1000), 'Single negative numbers return 0').to.be.equal(0);
        expect(Number().pseudoRandom(-1000, 1000), 'Negative numbers returns 0').to.be.equal(0);
        expect(Number().pseudoRandom(0), 'Single 0 return 0').to.be.equal(0);
    });
});
